...
async function runFind(
  ...
  if (!field) {
    console.error("Usage: okfa shot find <field> [--eq <v>] [--contains] [--regex]");
    return 2;
  }
...
async function runSearch(
  ...
  const query = argv[0];
  if (!query) {
    console.error("Usage: okfa shot search <query> [--regex] [--ignore-case] [--files-only] [--limit N]");
    return 2;
  }
...
async function runReplace(
  ...
  if (field) {
    if (!toValue && !fromValue) {
      console.error("Usage: okfa shot replace --field <k> [--from <v>] --to <v> [--match <v>] [--in-string] [--apply]");
      return 2;
    }
  ...
  if (!pattern) {
    console.error("Usage: okfa shot replace --pattern <p> --to <v> [--regex] [--apply]");
    return 2;
  }
...
export function explainShot(): string {
  return `Usage: okfa shot <ls|find|search|replace> [...]
